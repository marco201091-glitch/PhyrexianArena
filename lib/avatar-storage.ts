...
export function resolveAvatarUrl(
  ...
  state: AvatarObjectState | null,
  version: number,
) {
  if (!userId || !state?.exists || !state.objectName) return null;
  return getAvatarPublicUrl(client, userId, version, state.objectName, state.revision);
}
