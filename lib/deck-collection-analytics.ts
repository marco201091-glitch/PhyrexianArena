import { expandDeckCommanderNames } from '@/lib/deck-color-sync';
import {
  getCommanderOptions,
...
export function getDeckCommanderNames(deck: Pick<DeckCollectionSnapshot, 'commander' | 'commander_options'>) {
  ...
  return expandDeckCommanderNames(deck.commander);
}
...
export function collectUniqueCommanderNames(decks: DeckCollectionSnapshot[]) {
  const names = new Set<string>();
  decks.forEach((deck) => {
    getDeckCommanderNames(deck).forEach((name) => names.add(name));
  });
...
/** @deprecated Use buildAverageCommanderCmc */
export const buildAverageDeckCmc = buildAverageCommanderCmc;