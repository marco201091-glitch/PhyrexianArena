  ...
  return `
    <div style="font-family: Inter, Segoe UI, Arial, sans-serif; background:#050807; color:#f4f7f4; padding:32px 16px;">
      <div style="max-width:560px; margin:0 auto; background:#101713; border:1px solid #24592b; border-radius:16px; padding:28px;">
        ...
        ${content}
        <p style="margin:24px 0 0; font-size:12px; color:#8ca993;">21Life · Tracker &amp; Analytics</p>
      </div>
    </div>
  `.trim();
}
...
export function buildSignupConfirmationEmail(locale: AppLocale, actionLink: string): EmailTemplate {
  if (locale === 'en') {
    return {
      subject: 'Confirm your 21Life account',
      html: wrapEmailHtml(`
      ...
      `),
      text: `Confirm your 21Life account:\n${actionLink}`,
    };
  }

  return {
    subject: 'Conferma il tuo account 21Life',
    html: wrapEmailHtml(`
    ...
    `),
    text: `Conferma il tuo account 21Life:\n${actionLink}`,
  };
}
...
export function buildPasswordResetEmail(locale: AppLocale, actionLink: string): EmailTemplate {
  if (locale === 'en') {
    return {
      subject: 'Reset your 21Life password',
      html: wrapEmailHtml(`
      ...
      `),
      text: `Reset your 21Life password:\n${actionLink}`,
    };
  }

  return {
    subject: 'Reimposta la password di 21Life',
    html: wrapEmailHtml(`
    ...
    `),
    text: `Reimposta la password di 21Life:\n${actionLink}`,
  };
}
