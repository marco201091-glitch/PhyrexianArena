...
export function buildArenaShareText(payload: ArenaSharePayload, labels: ArenaShareLabels) {
  const lines: string[] = [
  ...
  payload.topDecks.slice(0, 5).forEach((deck, index) => {
    ...
    const owner = deck.ownerDisplayName ? ` — ${deck.ownerDisplayName}` : '';
    lines.push(`${index + 1}. ${deck.name || deck.commander}${owner}${bracket} - ${deck.winRate}% (${deck.wins}W / ${deck.gamesPlayed}G)`);
  });
  ...
  if (payload.recentMatches.length > 0) {
    ...
    payload.recentMatches.slice(0, 3).forEach((match) => {
      lines.push(`• ${format(new Date(match.playedAt), 'PPP')} - ${labels.winner}: ${match.winnerName}`);
