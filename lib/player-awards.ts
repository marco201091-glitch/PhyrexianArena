...
export function buildPlayerAwards(matches: Match[]): PlayerAward[] {
  const players = new Map<string, { name: string; games: number; wins: number; draws: number; second: number; eliminations: number; damage: number; first: number; combo: number; last: number; concession: number; skills: number }>();
  for (const match of matches) {
    ...
    for (const participant of match.match_participants) {
      ...
      const row = players.get(key) ?? { name: getParticipantDisplayName(participant), games: 0, wins: 0, draws: 0, second: 0, eliminations: 0, damage: 0, first: 0, combo: 0, last: 0, concession: 0, skills: 0 };
      ...
      row.eliminations += participant.eliminations_caused || 0; row.damage += participant.life_damage_dealt || 0; row.first += Number(Boolean(first && participant.eliminated_at === first));
  ...
  const definitions: Array<[PlayerAwardKind, (row: typeof rows[number]) => number, boolean]> = [
    ['veteran', r => r.games, false], ['eternal_second', r => r.second, false], ['arena_king', r => r.winRate, true], ['hitman', r => r.eliminations, false], ['berserker', r => r.damage, false], ['archenemy', r => r.first, false], ['combo', r => r.combo, false], ['last_standing', r => r.last, false], ['concession', r => r.concession, false], ['true_skills', r => r.skills, false],
  ];
