...
export default function ForgotPasswordScreen() {
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    try {
      const { error } = await apiPost('/api/auth/forgot-password', {
  ...
  return (
    <Screen background="solid">
      ...
      <View style={styles.form}>
        ...
        />
        <Button label={loading ? copy('loading') : copy('forgotPassword')} onPress={handleSubmit} disabled={loading} />
      </View>
    </Screen>
  );
}
...
const styles = StyleSheet.create({
  header: { alignItems: 'center', gap: 14, marginBottom: 20, marginTop: 12 },
