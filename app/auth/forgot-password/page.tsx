...
function ForgotPasswordForm() {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(payload.error || t({ it: 'Richiesta non riuscita', en: 'Request failed' }));
      }

      router.push(`/auth/check-email?mode=reset&email=${encodeURIComponent(email.trim())}`);
    } catch (error: unknown) {
      resetCaptcha();
      toast({
        title: t({ it: 'Errore', en: 'Error' }),
        description: error instanceof Error ? error.message : t({ it: 'Richiesta non riuscita', en: 'Request failed' }),
        variant: 'destructive',
      });
    } finally {
      setLoading(false);
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            >
              {loading
                ...
                : t({ it: 'Invia link di reset', en: 'Send reset link' })}
            </Button>
          </form>
