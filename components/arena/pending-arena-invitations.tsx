...
export function PendingArenaInvitations({ onAccepted }: { onAccepted?: (groupId: string) => void }) {
  ...
  return <div className="space-y-3">{items.map((item) => {
    ...
    return <Card key={item.id} className="border-violet-400/30 bg-violet-500/10"><CardContent className="flex flex-col gap-3 p-4 sm:flex-row sm:items-center"><Mail className="h-6 w-6 text-violet-300" /><div className="min-w-0 flex-1"><b>{copy({ it: 'Invito Arena', en: 'Arena invitation' })}: {group?.name}</b><p className="text-xs text-muted-foreground">{copy({ it: 'Da', en: 'From' })} {inviter?.display_name || `@${inviter?.username}`}</p></div><div className="flex gap-2"><Button variant="outline" size="sm" onClick={() => void respond(item, 'decline')} disabled={responding === item.id}><X className="mr-1 h-4 w-4" />{copy({ it: 'Rifiuta', en: 'Decline' })}</Button><Button size="sm" onClick={() => void respond(item, 'accept')} disabled={responding === item.id}>{responding === item.id ? <Loader2 className="mr-1 h-4 w-4 animate-spin" /> : <Check className="mr-1 h-4 w-4" />}{copy({ it: 'Accetta', en: 'Accept' })}</Button></div></CardContent></Card>;
  })}</div>;
}
